...
payload = {
    "flights": [
        ...
        },
        {
            ...
            "departure_city": "北京",
            "arrival_city": "上海",
            "departure_date": (today + timedelta(days=4)).isoformat(),
            "base_price": 1000,
            "discount_rate": 0.15
        },
        {
            ...
            "departure_city": "北京",
            "arrival_city": "上海",
            "departure_date": (today + timedelta(days=6)).isoformat(),
            "base_price": 1200,
            "discount_rate": 0.2
        }
    ],
...
print("=== 测试: POST /api/v1/packages/visualization ===")
try:
...
print("\n=== 测试: POST /api/v1/packages/recommend ===")
try:
    response = requests.post(
    ...
    if 'recommendations' in data and len(data['recommendations']) > 0:
        ...
        print(f"  排名: {rec['rank']}")
        print(f"  套餐ID: {rec['package_id']}")
        print(f"  原价: {rec['original_total_price']}")
        print(f"  折扣价: {rec['discounted_total_price']}")
        print(f"  折扣率: {rec['total_discount_rate']}")
        print(f"  推荐指数: {rec['recommendation_score']}")
except Exception as e:
    print(f"ERROR: {type(e).__name__}: {e}")
    import traceback
    traceback.print_exc()

print("\n=== API 测试完成，没有 IndexError ===")
